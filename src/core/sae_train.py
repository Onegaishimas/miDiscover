...
def inspect_model_layers(model, max_layers: int = 10):
    """Inspect model architecture to help identify correct layer names"""
    print(f"\n🔍 Model Architecture Inspection:")
    print("=" * 50)
    
    layer_count = 0
    for name, module in model.named_modules():
        if 'layers.' in name and ('mlp' in name or 'attn' in name):
            print(f"  {name}: {type(module).__name__}")
            layer_count += 1
            if layer_count >= max_layers:
                print(f"  ... (showing first {max_layers} relevant layers)")
                break
    
    # Try to find MLP structure
    try:
        # Check if it's Phi-2 structure
        test_layer = model.model.layers[0].mlp
        mlp_attrs = [attr for attr in dir(test_layer) if not attr.startswith('_') and not callable(getattr(test_layer, attr))]
        print(f"\n📋 MLP Layer Attributes: {mlp_attrs}")
        
        # Show actual layer structure
        print(f"\n🏗️  MLP Layer Structure:")
        for attr in ['fc1', 'fc2', 'down_proj', 'up_proj', 'gate_proj']:
            if hasattr(test_layer, attr):
                layer = getattr(test_layer, attr)
                if hasattr(layer, 'weight'):
                    print(f"  ✅ {attr}: {layer.weight.shape}")
                else:
                    print(f"  ✅ {attr}: {type(layer).__name__}")
        
        # Suggest correct layer names
        if hasattr(test_layer, 'fc2'):
            print(f"\n✅ Recommended layer: model.layers.16.mlp.fc2 (Phi-2 style)")
            print(f"   Alternative layers to try:")
            print(f"   - model.layers.8.mlp.fc2 (earlier layer)")
            print(f"   - model.layers.24.mlp.fc2 (later layer)")
        elif hasattr(test_layer, 'down_proj'):
            print(f"\n✅ Recommended layer: model.layers.8.mlp.down_proj (alternative style)")
        else:
            print(f"\n⚠️  Unknown MLP structure - check layer names manually")
            
    except Exception as e:
        print(f"\n⚠️  Could not inspect MLP structure: {e}")
    
    print("=" * 50)
    print(f"💡 Use one of the recommended layers with --layer-name argument")
    print("=" * 50)
...
def main():
    ...
    if args.use_db and not DB_AVAILABLE:
        ...
        print("Make sure PostgreSQL is running and db_utils.py is present.")
        return
    ...
    try:
        ...
        # Save features to database if enabled
        if training_run and config.use_db and DB_AVAILABLE:
            try:
                db = get_db()
                
                # Prepare features for database (exclude large decoder_weights)
                features_for_db = []
                for feature_info in feature_analysis["features"]:
                    features_for_db.append({
                        ...
                        "activation_frequency": feature_info["activation_frequency"],
                        "decoder_norm": feature_info["decoder_norm"]
                    })
                
                print(f"💾 Saving {len(features_for_db)} features to database...")
                
                # Use existing function to log features
                log_discovered_features(training_run.id, features_for_db, db)
                
                db.close()
                print(f"✅ All features saved to database")
                
            except Exception as e:
                print(f"⚠️ Warning: Could not save features to database: {e}")
        
        # Calculate metrics for database update
        # Use a more appropriate threshold for "active" features
        ...
        if training_run and config.use_db and DB_AVAILABLE:
            try:
                ...
                start_time = training_run.started_at
                if start_time:
                ...
                print(f"🔄 Updating training run {str(training_run.id)[:8]} completion status...")
                
                # Handle timezone-aware completion time
                if training_run.started_at and training_run.started_at.tzinfo is not None:
                    from datetime import timezone
                ...
                training_run.completed_at = completion_time
                training_run.status = 'completed'
                training_run.final_loss = final_loss
                training_run.sparsity_level = sparsity_percent
                training_run.active_features = active_features
                training_run.total_features = total_features
                training_run.training_duration_seconds = duration_seconds
                training_run.updated_at = update_time
                
                # Commit changes directly
                db.add(training_run)
                db.commit()
                db.refresh(training_run)  # Refresh to confirm update
                db.close()
                
                print(f"✅ Training run {str(training_run.id)[:8]} marked as completed")
                ...
                print(f"⚠️  Warning: Could not update training run status: {e}")
                print(f"   Error details: {type(e).__name__}: {str(e)}")
                print("   Training completed successfully, but database status not updated")
                # Close db session on error
                try:
                    db.close()
                except:
                    pass
        ...
        # Mark training run as failed if using database
        if training_run and config.use_db and DB_AVAILABLE:
            try:
                db = get_db()
                ...
                db.commit()
                db.close()
                print(f"🔴 Training run {str(training_run.id)[:8]} marked as failed")
